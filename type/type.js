...
// 문자는 홀따옴표와 겹따옴표 어떤 것을 사용해도 되나, 따옴표 안에 다른 따옴표가 들어간다면 오류가 생긴다.
const str = "'I'm waterrr'" + "'melon'";
console.log(str);
...
// - 전체 문자열을 백틱(` `)으로 감싸고, 변수 부분을 ${} 안에 작성한다
...
// 숫자형 문자는 + 의 경우 문자 연산이 되지만,
const f = 10;
console.log(d + f); //510으로 출력됨
const g = d + f;
console.log(typeof g); //type이 string(문자열)이기 때문

//나머지 연산은 엔진이 숫자로 임의 변경하여 연산한다. 이를 암묵적 형변환이라 한다.
...
// 4. NaN
// 자바스크립트 엔진이 숫자형으로 인식하는 것 중에 특이한 것은 NaN이다. 이는 Not a Number의 이니셜로 '숫자가 아님'이라는 뜻이지만 숫자다.
console.log(f * 'a');
...
// 7. null
// undefined와 같이 비어있는 값이라는 의미는 간다.
...
console.log(undefined === false); //undefined 역시 true, false, 0, 빈값으로 잡을 수 없다
console.log(null === 0);
